...
tribble(~title, ~coauthors,
        ...
        "Spatial Dependence and Overdispersion in Count Data", "Presented at Polmeth 2017") %>%
  detailed_entries(why = coauthors,what = title)
```

<!-- # Select Other Publications and Writing -->
...
tribble(~what,'Research Assistant') %>%
  detailed_entries(what = what,
                   why  = c('Ron Krebs - Fall 2019',
                            'John Freeman - Continuous spatial models',
                           'Jessica Stanton - Universal Jurisdiction and Civil War'))
...
tribble(~name,
        ...
        "Upper Midwest International Relations Conference (Minneapolis, MN): 2019",
        "BIGCSS Summer School in Computational Social Science (Breman, Germany): July 2018",
        "Upper Midwest International Relations Conference (Madison, WI): 2018") %>%
  brief_entries(what = name)
```
...
```{r}
tribble(~detail,
        "\\textbf{International Relations}: Political violence/conflict processes, health crises, disease management",
...
```{r}
tribble(~detail,
        "Spatial Econometrics, Mapping, Time Series, Bayesian Analysis, Visualisation, Data Cleaning, Forecasting, Surveys",
        ) %>%
